...
export default class TimeLogLists extends Component {
  ...
  render() {
    ...
    const timeLogsByDate = Object.keys(data).map(key => {
      ...
      return (
        <div className="px-0 my-4" key={key}>
          <div className="flex flex-row justify-between px-4">
            <h3 className="text-2xl hover:text-grey">{this.formatDate(key)}</h3>
            <span className="hover:text-teal">
              Total: {moment.utc(totalTimeMs).format('HH:mm:ss')}
            </span>
          </div>
          <TransitionGroup>{timeLogs}</TransitionGroup>
        </div>
      );
    });

    return <div className="flex flex-col mt-32">{timeLogsByDate}</div>;
  }
}
